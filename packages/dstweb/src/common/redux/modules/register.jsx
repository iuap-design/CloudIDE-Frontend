...
export function getSmsCode(data) {
  return function (dispatch) {
    ...
    fetch('/uniform/register/smscode', options)
...
export function checkSmsCode(data) {
  return function (dispatch) {
    ...
    const options = genFetchOptions('post', data);
    fetch('/uniform/register/checksmscode', options)
      .then(function (response) {
        if (response.status >= 400) {
          return {
            code: response.status,
  ...
  return function (dispatch) {
    ...
    fetch('/uniform/register/save', options)
      ...
      .then(function (json) {
        if (json.code != 200) {
          // dispatch(genAction('PLATFORM_DATA_CORP_REG_NEXT', json.message));
          dispatch(genAction('PLATFORM_DATA_REGISTER_SET_OPTIONS', { buttonLoading: false}))
          alert(json.message)
        } else {
          dispatch(genAction('PLATFORM_DATA_REGISTER_SET_OPTIONS', { buttonLoading: false}))
          dispatch(genAction('PLATFORM_DATA_CORP_REG_NEXT', json.message));
